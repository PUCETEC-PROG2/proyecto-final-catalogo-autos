...
class Compra(models.Model):
    ...
    precio_total = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, null=False)
    cliente = models.ForeignKey(Cliente, on_delete=models.RESTRICT)
    productos = models.ManyToManyField(Producto, through='LineaCompra')

    def calcular_precio_total(self):
        total = sum(lp.cantidad * lp.producto.precio for lp in self.lineas.all())
        return total
